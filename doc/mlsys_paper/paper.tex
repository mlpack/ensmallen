...
should operate only on mini-batches, and utility methods {\tt Shuffle()} and
{\tt NumFunctions()} must be added.  The same pattern applies for other
...
Lastly, we show the simplicity of using different optimizers for the same task.
Using a version of {\tt LinearRegressionFunction} adapted for separable
differentiable optimizers, we run six optimizers with default parameters in
eight lines of code:

% RC: I know this is inline code... but does it work here?  If not we can put it
% in a figure.
\begin{minted}[fontsize=\footnotesize]{c++}
  LinearRegressionFunction lrf(X, y); // X and y are data
...
This pluggability makes it easy to use {\tt ensmallen} to generate comparisons
between optimizers, and in Figure \ref{fig:learning_curve} we use these six
lines of code to generate learning curves for each of these six optimizers on
the {\tt YearPredictionMSD} dataset from the UCI dataset
repository~\cite{ucimlrepository}. % TODO: add to bibtex
In addition, any other optimizers for separable differentiable objective
functions could be dropped into place.

\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{experiments/learning_curves.png}
\label{fig:learning_curve}
\caption{Learning curves for six {\tt ensmallen} optimizers on {\tt
YearPredictionMSD} dataset over 5 epochs of training.  Optimizers can be
