...
For the most common case of a differentiable $f(x)$, the user must
...
\begin{minted}[fontsize=\footnotesize]{python}
    def rosen(x): return 100.0 * (x[1] - x[2]**2.0)**2.0 + (1 - x[2])**2.0)
...
very many times; for each simulation we limit the optimizer to $100,000$
