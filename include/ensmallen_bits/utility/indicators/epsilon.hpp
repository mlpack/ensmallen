 ...
 *
 * Epsilon indicator: A binary quality indicator that is capable of detecting
 * whether one approximation set is better than another.
 *
...
/**
 * The epsilon indicator is one of the binary quality indicators that was
 * proposed by Zitzler et. al.. The indicator originally calculates a weak
 * dominance relation between two approximation sets. It returns "epsilon" which
 * is the factor by which the given approximation set is worse than the
 * reference front with respect to all the objectives.
 *
                                               ...
                                               const CubeType& referenceFront)
  {
    ...
    for (size_t i = 0; i < referenceFront.n_slices; ++i)
    {
      ...
      for (size_t j = 0; j < front.n_slices; ++j)
      {
        ...
        frontRatio.replace(arma::datum::inf, -1.0);
